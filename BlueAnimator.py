...
class Exporter:
    ...
    def export(self, canvas, app):
        ...
        width, height = hsize - posx, vsize - posy
...
class Canvas:
    ...
    def __mouseInCanvas(self):
        ...
        if (
            self.posx + self.thickness // 2
            <= mx
            <= self.posx + self.HSize - self.thickness // 2
        ):
            if (
                self.posy + self.thickness // 2
                <= my
                <= self.posy + self.VSize - self.thickness // 2
            ):
                self.mouseinc = True
                return
        self.mouseinc = False
        ...
        if self.mouseinc:
            ...
            if event.type == pygame.MOUSEMOTION:
                if pygame.mouse.get_pressed()[0]:
                    self.frames[self.frame][len(self.frames[self.frame]) - 1].append(
                        (pygame.mouse.get_pos(), self.thickness)
                    )
    ...
    def show(self, frame):
        ...
        pygame.draw.rect(
            frame.window, WHITE, ((self.posx, self.posy), (self.HSize, self.VSize))
        )
        pygame.draw.rect(
            frame.window,
            BLACK,
            ((self.posx, self.posy), (self.HSize, self.VSize - self.posy)),
            10,
        )
        if 0 < self.frame:
            for innerlst in self.frames[self.frame - 1]:
        ...
        for innerlst in self.frames[self.frame]:
            for i in range(0, len(innerlst), 1):
                if i < len(innerlst) - 1:
                    ...
                    pygame.draw.circle(
                        frame.window, self.color, innerlst[i][0], innerlst[i][1] // 2
                    )
        if self.mouseinc:
            pygame.draw.circle(
                frame.window,
                (
                    abs(180 - self.color[0]),
                    abs(180 - self.color[1]),
                    abs(180 - self.color[2]),
                ),
...
class UIColorSelector:
    ...
    def show(self, frame):
        if self.showColorRect:
            frame.window.blit(
                self.colorImg, (self.posx + self.thickness, self.posy + self.thickness)
            )
            radius = self.HSize // 2 * 2**0.5 + self.thickness
            pygame.draw.circle(
                frame.window,
                BLACK,
                (self.posx + self.HSize // 2, self.posy + self.VSize // 2),
                radius,
                self.thickness,
            )
            angle = (((360 / 255) * self.blue) / 180) * math.pi
            circlex = (
                math.cos(angle) * (radius - self.thickness // 2)
                + self.posx
                + self.HSize // 2
            )
            circley = (
                -math.sin(angle) * (radius - self.thickness // 2)
                + self.posy
                + self.VSize // 2
            )
            pygame.draw.circle(
                frame.window, WHITE, (circlex, circley), self.thickness * 2
            )
        ...
        if self.showColorBars:
            while self.rgb[len(self.rgb) - 1] < 3:
                ...
                pygame.draw.circle(
                    ...
                    (
                        self.x1
                        + self.barLength * self.rgb[self.rgb[len(self.rgb) - 1]] / 255,
                        self.y,
                    ),
                    self.thickness * 2,
                )
        ...
        pygame.draw.rect(
            ...
            (
                (self.posx, self.posy),
                (
                    self.thickness + self.HMargin + self.barLength,
                    self.VMargin * 4 + self.thickness * 2,
                ),
            ),
            self.thickness,
        )
...
class UIMenu:
    def __init__(self, posx, posy, HSize, thickness, HMargin, VMargin):
        self.thickness = thickness
        self.HSize, self.VSize = HSize, self.thickness
    ...
    def handleEvents(self, event):
        ...
        clicked = False
        events = {"clicked": False, "ishovering": False}
        entry, onhover = None, None
        self.HOffset = self.HMargin + self.thickness
        if self.onResizef != None:
            if event.type == (
                pygame.WINDOWRESIZED or pygame.WINDOWMAXIMIZED or pygame.WINDOWMINIMIZED
            ):
                self.onResizef(self)

        if self.__mouseOnBar(mousex, mousey):
            for entry_element in self.entries:
                onhover = self.__mouseOnEntry(
                    entry_element, mousex, mousey, self.HOffset
                )
                entry = entry_element
                if onhover:
                    ...
                    if self.notOnHoverf != None:
                        self.notOnHoverf(entry)

                self.HOffset += (
                    self.HMargin * 2 + self.thickness + entry.textImg.get_width()
                )
    ...
    def show(self, frame):
        ...
        pygame.draw.rect(
            frame.window,
            BLACK,
            ((self.posx, self.posy), (self.HSize, self.VSize)),
            self.thickness,
        )
...
class MainApp:
    def __init__(self, size: tuple):
        ...
        pygame.display.set_caption("BlueAnimator")
...
def entryClicked(entry):
    if entry.text == "Exit":
        pygame.quit()
        sys.exit(0)
    elif entry.text.startswith("FPS"):
        canvas.FPS += 1
        entry.text = "FPS:" + str(canvas.FPS)
    elif entry.text == ">":
        canvas.addFrame()
        canvas.frame += 1
...
menu = UIMenu(0, 0, app.width, 8, 8, 8)
