        ...
        if self.mouseinc:
            ...
            if event.type == pygame.MOUSEMOTION:
                if pygame.mouse.get_pressed()[0]:
                    self.frames[self.frame][len(self.frames[self.frame])-1].append((pygame.mouse.get_pos(),self.thickness))
        ...
        if 0 < self.frame:
            for innerlst in self.frames[self.frame-1]:
                for i in range(0, len(innerlst), 1):
                    if i < len(innerlst) - 1:
                        pygame.draw.line(frame.window,LIGHTGRAY,innerlst[i][0],innerlst[i+1][0],innerlst[i+1][1])
                        pygame.draw.circle(frame.window,LIGHTGRAY,innerlst[i][0],innerlst[i+1][1]//2)
        for innerlst in self.frames[self.frame]:
            for i in range(0, len(innerlst), 1):
                if i < len(innerlst) - 1:
                    pygame.draw.line(frame.window,self.color,innerlst[i][0],innerlst[i+1][0],innerlst[i][1])
                    pygame.draw.circle(frame.window,self.color,innerlst[i][0],innerlst[i][1]//2) 
        if self.mouseinc:
